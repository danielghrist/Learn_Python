...
def write_state_to_screen(turtle, player_guess):
    ...
    turtle.write(arg=player_guess, align="center", font=("Arial", 10, "normal"))


# Retun True if player's guess was in the DataFrame of states
def check_player_guess(player_guess, df):
    states = df.state.to_list()
    for state in states:
        if player_guess == state:
            return True
    return False
...
while not is_game_over:
    player_answer = screen.textinput(title=text_input_title, prompt="Guess the name of a state").title()
    ...
    if player_answer == "Exit":
        ...
        not_guessed_states = [state for state in states if state not in correct_answers]
        turtle.write(f"You guessed {num_correct_answers} out of {total_states} states.\n", align="center", font=("Arial", 25, "bold"))
        turtle.write(f"The states you missed have been saved", align="center", font=("Arial", 25, "bold"))
    ...
    if check_player_guess(player_answer, state_data):
    ...
    if num_correct_answers == 50:
        turtle.home()
        turtle.write("You guessed all the states.  You win!", align="center", font=("Arial", 25, "bold"))
        is_game_over = True
